...
export default function EditProject({ params }) {
        ...
        const fetchProject = async () => {
            try {
                ...
                if (data.galleryImages && Array.isArray(data.galleryImages)) {
                    setGalleryPreviews(data.galleryImages.map(url => ({ type: 'url', src: url })));
                }
    ...
    const handleGalleryChange = (e) => {
        ...
        files.forEach(file => {
            const reader = new FileReader();
            reader.onloadend = () => {
                setGalleryPreviews(prev => [...prev, { type: 'file', src: reader.result, file }]);
            };
            reader.readAsDataURL(file);
        });
    };
                                ...
                                <div className="flex flex-wrap gap-4 mt-2">
                                    {galleryPreviews.map((item, index) => (
                                        <div key={index} className="relative group">
                                            <img src={item.src} alt={`Gallery ${index}`} className="h-20 w-20 object-cover rounded-lg border" />
                                            <button
                                                type="button"
